...
def print_notes(notes):
    ...
    for i, note in enumerate(notes):
        print(grey(f'{i + 1}. ') + f'#{note.id} {note.dtime} {note.header_preview(40)} {note.txt_preview(180)}')

def edit_note(note):
    os.system('cls')
    print(grey('Предыдущий заголовок: ') + f'{note.header}')
    note.header = input(grey('Новый заголовок: '))
    print(grey('Предыдущий текст:\n') + f'{note.txt}')
...
def main_menu():
    ...
    while True:
        ...
        if input_num == '1.1':  # Вывести сначала новые
            ...
            i = int(input(grey('Выберите нужную заметку: ')))
            if 0 < i <= len(sorted_notes):
                note_menu(sorted_notes[i - 1])
        elif input_num == '1.2':  # Вывести сначала предыдущие
            ...
            os.system('cls')
            start_date = input(grey('Дата начала периода (дд.мм.гг чч:мм:сс): '))
            end_date = input(grey('Дата окончания периода (дд.мм.гг чч:мм:сс): '))
